...
MainFrame.Size = UDim2.new(0, 320, 0, 760) 
...
local function checkTargetVisibility(targetPart, targetChar)
    ...
    while true do
        ...
        local isWallbangable = WallbangableMaterials[mat] or hitInstance.Transparency >= 0.8 or 
                               hitInstance.Name:lower():find("grass") or hitInstance.Name:lower():find("glass") or 
                               hitInstance.Name:lower():find("ignore")
...
local function GetBestTargetInFOV()
    ...
    for entity, box in pairs(ESP_Objects) do
        ...
        if char and char ~= LocalPlayer.Character and char.Parent then
            local head = char:FindFirstChild("Head") or char:FindFirstChild("HumanoidRootPart") or 
...
local function GetValuableMatch(desc)
    ...
    if ESP_Config.FindWeapons then
        local weaponKeywords = {"golden", "mp5sd", "val", "asval", "m4a1", "m4", "fn-fal", "fal", "svd", "pkm", "r700", "remington", "tfz", "mod-98", "mod98", "rpg-7", "rpg7", "akmn", "saiga", "flare", "flaregun", "spsh-44", "spsh44"}
    ...
    if ESP_Config.FindValuables then
        if nLower:find("mount") or nLower:find("helmet") or nLower:find("headset") then
            ...
            for _, optic_kw in ipairs(highValueOptics) do
                if nLower:find(optic_kw) then return true, "HIGH-VALUE NIGHT VISION MOUNT" end
            end
        end
        local valuableKeywords = {"solter", "gold", "sps", "watch", "tix", "ticket", "cpu", "ram", "ssd", "gpu", "smartphone", "phone", "ruble", "rubles", "cash", "money", "nvg", "goggles", "reap-ir", "reapir", "thermal", "onv-9", "onv9", "quadnvg", "quad"}
    ...
    if ESP_Config.FindKeys then
        local keyKeywords = {"key", "keycard", "card", "access", "secure", "dorm", "marked", "factory", "resort", "lab", "sanitar", "manager", "kiba", "bunker", "garage", "airfield", "village", "fueling", "atc", "crane", "b-05", "frigate", "evac", "villa", "hydropower", "power", "apartment", "pool"}
        }
    ...
    if ESP_Config.FindEquipment then
        -- Database armor, tas, dan visor kasta tertinggi hasil riset ingame
        local equipKeywords = {
...
local function checkContainerLoot(container)
    ...
    for _, desc in ipairs(container:GetDescendants()) do
        local isContainerPart = desc.Name:lower():find("mount") or desc.Name:lower():find("keypad") or 
                                desc.Name:lower():find("hinge") or desc.Name:lower():find("door") or 
                                desc.Name:lower():find("lock") or desc.Name:lower():find("frame") or 
                                desc.Name:lower():find("drawer")
...
task.spawn(function()
    ...
    while task.wait(2) do
        ...
        local function ScanObject(obj, scanCategory)
            ...
            if nameLower:find("poster") or nameLower:find("decal") or nameLower:find("sign") or nameLower:find("prop") or 
               nameLower:find("static") or nameLower:find("building") or nameLower:find("foliage") then
                return
            end
                ...
                if not isPlayerChar then
                    ...
                    if ESP_Objects[obj] then
                        if not ESP_Objects[obj].IsContainer and not ESP_Objects[obj].IsLooseItem then
                            -- OVERWRITE INSTAN: Musuh mati
                            if isDead and not ESP_Objects[obj].IsDeadCache then
                                RemoveESP(obj)
                                ESP_Objects[obj] = nil
                                task.defer(function()
                                    CreateESP(obj, false)
                                    if ESP_Objects[obj] then ESP_Objects[obj].IsDeadCache = true end
                                end)
                            -- LOGIKA RESPAWN: Musuh hidup kembali
                            elseif not isDead and ESP_Objects[obj].IsDeadCache then
                                RemoveESP(obj)
                                CreateESP(obj, false)
                                if ESP_Objects[obj] then
                                    ESP_Objects[obj].IsDeadCache = false
                    ...
                    if hasLoot then
                        if not ESP_Objects[obj] then
                            ...
                            if adorneePart then
                                ...
                                txt.TextColor3 = Color3.fromRGB(255, 215, 0); txt.Font = ESP_Config.Font; txt.TextSize = 13
                                Instance.new("UIStroke", txt).Thickness = 2
                                
                                local hl = Instance.new("Highlight")
                                hl.Name = "LootHighlight"; hl.FillColor = Color3.fromRGB(212, 175, 55); hl.OutlineColor = Color3.fromRGB(212, 175, 55)
                    ...
                    if not isMatch then
                        for _, desc in ipairs(obj:GetDescendants()) do
                            if desc:IsA("StringValue") and desc.Value and desc.Value ~= "" then
                                ...
                                if isValMatch then isMatch = true; matchName = desc.Value; break end
                            end
                        end
                    end
                    if isMatch then
                        ...
                        local txt = Instance.new("TextLabel", bb)
                        txt.Size = UDim2.new(1,0,1,0); txt.BackgroundTransparency = 1; txt.Text = "[ " .. matchName .. " ]"
                        txt.TextColor3 = Color3.fromRGB(255, 215, 0); txt.Font = ESP_Config.Font; txt.TextSize = 13
                        Instance.new("UIStroke", txt).Thickness = 2
            ...
            if ESP_Config.PerformanceMode then
                ...
                task.spawn(function()
                    ...
                    local function ScanPerformance(obj)
                        ...
                        for _, child in ipairs(obj:GetChildren()) do ScanPerformance(child) end
                    end
