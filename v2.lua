...
-- // Environmental Analytics Setup (Dideklarasikan di Atas Agar Bebas Error)
...
MainFrame.Size = UDim2.new(0, 320, 0, 610) -- Ukuran ditinggikan ke 610 agar muat toggle baru
...
local function CreateToggle(labelText, configKey)
    ...
    Btn.MouseButton1Click:Connect(function()
        ESP_Config[configKey] = not ESP_Config[configKey] -- FIX BUG: Perbaikan typo configkey
...
-- // UTAS 1: Ambil Kecepatan Peluru Dinamis dari Modul Project Delta
...
-- // UTAS 2: Fungsi Vis-Check Perbaikan Skema Warna (Bypass Dinding Lunak)
local function checkTargetVisibility(targetPart)
    ...
    if WallbangableMaterials[raycastResult.Material] or hitInstance.Transparency > 0.5 or not hitInstance.CanCollide then
        return "Wallbang", Color3.fromRGB(255, 255, 255) -- Terpadu Putih Cerah
    else
        return "Blocked", Color3.fromRGB(80, 80, 80)    -- Tembok Solid = Abu-abu Gelap (Warna Mayat)
    end
end

-- // UTAS 3: Seleksi Target Prioritas Kursor Layar 2D (Bypass Jarak Studs)
...
-- // Weapon Utilities (Chams & Bullet Tracer)
...
-- // GABUNGAN SCANNER GANDA: Player dan Bot Aktif Berdampingan
task.spawn(function()
...
-- // UTAS 4: Render Loop Utama Terintegrasi Balistik & Break Lock
