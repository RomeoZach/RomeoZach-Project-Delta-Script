...
local success, err = pcall(function()
    ...
    RunService:BindToRenderStep("RomeoZach_Render", 2005, function(deltaTime)
        ...
        for entity, box in pairs(ESP_Objects) do
            ...
            local isLootRange = (studsDist <= 17.85) -- 5 meter

            local shouldRender = false
            if isDead then
                shouldRender = (studsDist <= 267.85) -- Max 75m untuk mayat
            else
                local isPlayerChar = Players:GetPlayerFromCharacter(char) ~= nil
                if isPlayerChar then shouldRender = (studsDist <= 3571.4) else shouldRender = (studsDist <= 1607.1) end
            end
