            ...
            if rootPart then
                ...
                if shouldRender then
                    ...
                    if box.IsHelicopter then
                        ...
                        -- 1. MATIKAN TEKS NAMA (Nama dihapus total agar tidak memenuhi layar)
                        if box.NameLabel then 
                            box.NameLabel.Text = ""
                            box.NameLabel.Visible = false
                        end
                        
                        -- 2. TAMPILKAN HANYA METERAN JARAK SECARA KONSISTEN
        ...
        elseif ESP_Config.Enabled and isItem then
            ...
            if box.Billboard or box.Highlight then
                ...
                if itemPos then
                    ...
                    if inRange then
                        -- =======================================================
                        -- LOGIKA RENDERING UTAMA UNTUK LOOT CONTAINER (MURNI BOX)
                        -- =======================================================
                        if box.IsContainer then
                            -- Kontainer hanya menyala emas jika TERBUKTI memiliki loot berharga (HasLoot = true)
                            -- Dan visual otomatis mati jika Anda sudah sangat dekat (di bawah 4 studs) untuk mengambil loot
                            if box.HasLoot and studsDist > 4 then
                                -- Nyalakan ESP boks fisik warna Emas Murni tanpa kedip
                                if box.Highlight then 
                                    box.Highlight.Enabled = true 
                                ...
                                -- [SOLUSI MUTLAK] Kosongkan teks dan matikan BillboardGui agar tidak ada tulisan apa pun
                                if box.DistLabel then box.DistLabel.Text = "" end
                                if box.Billboard then box.Billboard.Enabled = false end
                                if box.DistBillboard then box.DistBillboard.Enabled = false end
                            else
                                -- Matikan total visual emas jika kontainer kosong / berisi sampah / sudah didekati player
