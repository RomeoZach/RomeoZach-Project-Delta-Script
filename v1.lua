...
local function CreateESP(entity, isPlayer)
    ...
    local function ApplyVisuals(char)
        ...
        box.IsDeadCache = false -- Reset status cache kematian
        ...
        bb.StudsOffset = Vector3.new(0, 1.5, 0) -- Mengangkat teks menjauhi dada/wajah target
        box.Billboard = bb
        
        local nameTxt = Instance.new("TextLabel", bb)
        nameTxt.Size = UDim2.new(1, 0, 0, 20); nameTxt.Position = UDim2.new(0, 0, 0, -5); nameTxt.BackgroundTransparency = 1
        nameTxt.Text = isPlayer and (entity.DisplayName or entity.Name) or entity.Name
        nameTxt.TextColor3 = COLOR_VISIBLE; nameTxt.TextSize = 10; nameTxt.Font = ESP_Config.Font; nameTxt.TextStrokeTransparency = 0; box.NameLabel = nameTxt
        nameTxt.TextYAlignment = Enum.TextYAlignment.Bottom
        Instance.new("UIStroke", nameTxt).Thickness = 1.5
        ...
        local function ScanObject(obj, scanCategory)
                    ...
                    if ESP_Objects[obj] then
                        ...
                        if not ESP_Objects[obj].IsContainer and not ESP_Objects[obj].IsLooseItem then
                            ...
                            if isDead and not ESP_Objects[obj].IsDeadCache then
                                RemoveESP(obj)          -- Bersihkan memori dan hapus visual lama
                                CreateESP(obj, false)   -- Paksa buat visual baru (mereset Highlight) agar tak nyangkut
                                if ESP_Objects[obj] then
                                    ESP_Objects[obj].IsDeadCache = true -- Kunci status agar warnanya meledak di Render Loop
            ...
            elseif scanCategory == "Container" then
                ...
                local containerKeywords = {
                    ...
                    "vault", "islootable", "crate", "sport", "airdrop", "bag", "drawer", "jacket", "case" -- Backup fallback
                }
            ...
            if rootPart then
                ...
                if shouldRender then
                    ...
                    if box.IsHelicopter then
                        ...
                        if box.NameLabel and not isDead then 
                            box.NameLabel.TextColor3 = finalColor
                            box.NameLabel.TextSize = dynamicTextSize
                            if box.Billboard and box.Billboard.Adornee ~= rootPart then box.Billboard.Adornee = rootPart end 
                        end
                ...
                if shouldRender then
                    ...
                    if box.Billboard and not box.IsHelicopter then box.Billboard.Enabled = not isDead end
                    
                    if box.DistBillboard then
                        if box.IsHelicopter or isDead then
